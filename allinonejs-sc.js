...
/** Togglespoiler **/
function togglespoiler (postid){var whichpost = document.getElementById(postid);if (whichpost.className=="widgetshown"){whichpost.className="widgethidden";} else{whichpost.className="widgetshown";}}
/* ContainerSelect */
function containerSelect(id) { containerUnselect(); if(document.selection) { var range = document.body.createTextRange(); range.moveToElementText(id); range.select();}
else if(window.getSelection) { var range = document.createRange(); range.selectNode(id); window.getSelection().addRange(range);}}
function containerUnselect() { if (document.selection) { document.selection.empty(); } else if (window.getSelection) { window.getSelection().removeAllRanges(); }}
...
/* text-slide */
  var list = $('news-feed').getFirst('ul'); var items = list.getElements('li'); var showDuration = 3000; var scrollDuration = 500; var index = 0; var height = items[0].getSize().y;
  var move = function() { list.set('tween',{ duration: scrollDuration, onComplete: function() { if(index == items.length - 1) { index = 0 - 1; list.scrollTo(0,0); } } }).tween('top',0 - (++index * height)); };
  window.addEvent('load',function() { move.periodical(showDuration);});
/* v-toggle */
  var myVerticalSlide = new Fx.Slide('v_juljol').hide(); $('v_toggle').addEvent('click', function(event){ event.stop(); myVerticalSlide.toggle(); }); $('v_nyumput').addEvent('click', function(event){ event.stop(); myVerticalSlide.slideOut(); });
});
...
/*widget list */
  jQuery('.widget-content li a, .eusi-popular-posts li a').hover(function() { jQuery(this).animate({ paddingLeft: '7px' }, {duration:300}); }, function() { jQuery(this).animate({ paddingLeft: '0px' }, {duration:300}); });
/* sociable img */
...
/* sharethis_toolbox, stickysocial, luhur-handap */
  jQuery('.sharethis_toolbox img, .stickysocial img, .luhur-handap img').fadeTo('slow', 0.75); jQuery('.sharethis_toolbox img, .stickysocial img, .luhur-handap img').hover(function() { jQuery(this).fadeTo('slow', 1); }, function() { jQuery(this).fadeTo('slow', 0.75); });
});
/* Scrolltopdown */
